...
# This script calculates the maxPods value to be used when starting up the kubelet.
# Reference https://github.com/awslabs/amazon-eks-ami/blob/main/templates/al2/runtime/max-pods-calculator.sh
...
# hardcoding to 1.10.0
CNI_VERSION="${CNI_VERSION:-1.10.0}"
...
SHOW_MAX_ALLOWED="${SHOW_MAX_ALLOWED:-true}"
...
if [ "$SHOW_MAX_ALLOWED" = true ]; then
  echo "Max Pods Allowed: "
  echo $max_pods
fi

if [ "$CPU_COUNT" -gt 30 ]; then
  echo "CPU_COUNT is 30 or less Max Pods Recommended: "
  echo $(min_number $MAX_POD_CEILING_FOR_HIGH_CPU $max_pods)
else
  echo "CPU_COUNT is greater than 30 Max Pods Recommended: "
